<?php
/**
 * DocsPress_Block_Template_Utils class used for serving block templates from Docs Blocks.
...
/**
 * DocsPress_Block_Template_Utils class
 */
class DocsPress_Block_Template_Utils {
    /**
    ...
    public static function gutenberg_build_template_result_from_post( $post ) {
        ...
        if ( ! $terms ) {
            return new \WP_Error( 'template_missing_theme', __( 'No theme is defined for this template.', 'docspress' ) );
        }
    ...
    public static function gutenberg_build_template_result_from_file( $template_file, $template_type ) {
        ...
        $template->theme   = $template_is_from_theme ? $theme_name : ( 'docspress' === self::PLUGIN_SLUG ? 'DocsPress' : self::PLUGIN_SLUG );
    ...
    /**
     * Build a new template object so that we can make Docs Blocks default templates available in the current theme should they not have any.
     *
     ...
     * @param bool   $template_is_from_theme If the block template file is being loaded from the current theme instead of Docs Blocks.
     *
    ...
    public static function create_new_block_template_object( $template_file, $template_type, $template_slug, $template_is_from_theme = false ) {
        ...
        $new_template_item = array(
            ...
            'path'        => $template_file,
            'type'        => $template_type,
            'theme'       => $template_is_from_theme ? $theme_name : ( 'docspress' === self::PLUGIN_SLUG ? 'DocsPress' : self::PLUGIN_SLUG ),
            // Plugin was agreed as a valid source value despite existing inline docs at the time of creating: https://github.com/WordPress/gutenberg/issues/36597#issuecomment-976232909.
    ...
    public static function convert_slug_to_title( $template_slug ) {
        switch ( $template_slug ) {
            case 'single-docs':
                return __( 'Single DocsPress', 'docspress' );
            case 'archive-docs':
                return __( 'Archive DocsPress', 'docspress' );
            default:
     ...
     *
     * @param string $template_name name of the template file without .html extension e.g. 'single-docs'.
     * @return boolean
     */
    ...
    public static function template_is_eligible_for_docs_archive_fallback( $template_slug ) {
        $eligible_for_fallbacks = array( 'taxonomy-docs_cat', 'taxonomy-docs_tag' );

        return in_array( $template_slug, $eligible_for_fallbacks, true )
            && ! self::theme_has_template( $template_slug )
            && self::theme_has_template( 'archive-docs' );
    }
