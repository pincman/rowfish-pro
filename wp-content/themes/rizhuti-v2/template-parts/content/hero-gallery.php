	...
	<?php foreach ( $gallery_ids as $key => $image_id ) {
		...
		$thum = (!empty($thum_1)) ? $thum_1 : $full ;
	  	...
	  	if ($key>=$free_num && !$IS_PAID && !$is_nologin_free) {
	  		...
	  		$thum_2 = (!empty($thum_1)) ? $thum_1 : array(_get_post_thumbnail_url(),300,200);
	  		$thum = $thum_2;
	  		$full = array('#',0,0);
...
jQuery(function() {
    'use strict';
    jQuery(".entry-gallery.justified-gallery").justifiedGallery({border:0,margins:6,rowHeight:140,captions:!1}),jQuery(".entry-gallery.justified-gallery").on("click",".gallery-item > a",function(e){e.preventDefault();var r,a,t=[];r=(a=$(this)).parent().index(),jQuery.each(a.parent().siblings().addBack(),function(e,r){var a=jQuery(r).find("a").attr("href"),i=jQuery(r).find("img").attr("src"),l=jQuery(r).find("a").attr("alt")||jQuery(r).find(".caption").text();"#"!==a&&t.push({src:a,thumb:i,subHtml:l})}),a.lightGallery({dynamic:!0,dynamicEl:t,download:!1,share:!1}).data("lightGallery").index=r});
});
</script>