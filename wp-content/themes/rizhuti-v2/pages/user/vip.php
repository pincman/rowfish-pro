        ...
        <div class="col-lg-4 mb-3">
            <div class="pay-vip-item card shadow" data-type="<?php echo $type;?>" data-price="<?php echo $opt['price'];?>">
              <div class="vip-body">
                ...
                <p class="vip-text day small text-muted"><?php echo site_mycoin('name');?>支付仅需：<i class="<?php echo site_mycoin('icon');?>"></i> <?php echo convert_site_mycoin($opt['price'],'coin');?></p>
                <p class="vip-text small text-muted">每日可免费下载<?php echo $opt['downnum'];?>个VIP资源</p>
              </div>
            </div>
        </div>
    <?php } ?>
...
<div class="card mt-0 mt-lg-3">
    <div class="card-header">
        <h5 class="card-title"><i class="fa fa-diamond nav-icon"></i><?php echo esc_html__('开通VIP会员，尊享VIP免费特权','rizhuti-v2');?></h5>
    </div>
        ...
        <table class="table mb-0 text-nowrap">
          <thead>
            <tr class="text-uppercase text-body">
              <th scope="col" class="font-weight-bold">等级</th>
              <th scope="col" class="font-weight-bold">每日可免费下载</th>
              <th scope="col" class="font-weight-bold">下载速度kb/s</th>
              <th scope="col" class="font-weight-bold">会员时长/天</th>
            </tr>
          </thead>
          <tbody class="text-dark">
            <?php $vip_opt = _get_ri_vip_options();
            foreach ($vip_opt as $key => $item) {
                echo '<tr>';
                echo '<th scope="row">'.$item['name'].'</th>';
                echo '<td class="align-middle border-top-0">'.$item['downnum'].' 个'.$item['name'].'资源</td>';
                if (empty($item['download_rate']) || $item['download_rate']==0) {
                    echo '<td class="align-middle border-top-0">不限速</td>';
                }else{
                    echo '<td class="align-middle border-top-0">'.$item['download_rate'].' KB/秒</td>';
                }
                echo '<td class="align-middle border-top-0">'.$item['day'].'天</td>';
                echo '</tr>';
            }?>
