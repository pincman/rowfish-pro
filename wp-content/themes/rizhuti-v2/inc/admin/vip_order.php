...
if ($page_action=='delete_log') {
	$wpdb->delete( "$wppay_table_name", array('order_type' => 4,'status' => 0));
	$wpdb->delete( "$wppay_table_name", array('order_type' => 3,'status' => 0));
	$wpdb->delete( "$wppay_table_name", array('order_type' => 2,'status' => 0));
...
<!-- 主页面 -->
<div class="wrap">
	<h1 class="wp-heading-inline">会员开通订单记录</h1>
...
class RiPlus_List_Table extends WP_List_Table{
    ...
    private function table_data($per_page = 5, $page_number = 1 ){
		...
		$sql = "SELECT * FROM $wppay_table_name WHERE order_type>=2 AND order_type<=4";
		//根据用户查询
		if ( ! empty( $_REQUEST['s'] ) ) {
			$user_id = 0;
    ...
    private function table_data_count() {
        ...
        $sql = "SELECT COUNT(*) FROM $wppay_table_name WHERE order_type>=2 AND order_type<=4";
		//根据用户查询
		if ( ! empty( $_REQUEST['s'] ) ) {
			$user_id = 0;
			if (is_numeric($_REQUEST['s'])) {
