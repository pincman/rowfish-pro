...
if ($page_action=='delete_log' && $wpdb->delete( "$wppay_table_name", array('order_type' => 1,'status' => 0))) {
...
<!-- 主页面 -->
<div class="wrap">
	<h1 class="wp-heading-inline">文章资源订单记录</h1>
...
class RiPlus_List_Table extends WP_List_Table{
    ...
    private function table_data($per_page = 5, $page_number = 1 ){
		...
		$sql = "SELECT * FROM $wppay_table_name WHERE order_type=1";
		//根据用户查询
		if ( ! empty( $_REQUEST['s'] ) ) {
			$user_id = 0;
    ...
    private function table_data_count() {
        ...
        $sql = "SELECT COUNT(*) FROM $wppay_table_name WHERE order_type=1";
		//根据用户查询
		if ( ! empty( $_REQUEST['s'] ) ) {
			$user_id = 0;
			if (is_numeric($_REQUEST['s'])) {
