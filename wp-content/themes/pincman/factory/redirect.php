...
/*
 * @Author: Pincman
 * @created_at: 2021-05-21 07:05:10
 * @updated_at: 2021-05-21 07:05:33
 * @description: 自定义跳转页面
...
if (isset($_GET['down']) && !isset($_GET['url'])) {
    ...
    // 开始下载处理
    $download_limit = round((float) _cao('download_limit') * 1024 * 1000);
    ...
    $is_down_free = $post_down_info[$down_post_key]['free'] == '1';
    $is_down_online = $post_down_info[$down_post_key]['online'] == '1';
    // //是否购买
    ...
    $single_down_free = (_cao('free_onlogin_down') == '1' && $is_down_free) || (!_cao('free_onlogin_down') && $uinfo['user_id'] && $is_down_free);
    $can_download = $uinfo['can'] || $single_down_free;
    if (!$can_download) {
        if (!$uinfo['user_id']) {
