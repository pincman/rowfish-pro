 ...
 * 侧边栏分类文章展示
 */
CSF::createWidget('pm_widget_series_posts', array(
    'title'       => esc_html__('PM: 专题文章侧边栏', 'rizhuti-v2'),
...
if (!function_exists('pm_widget_series_posts')) {
    function pm_widget_series_posts($args, $instance)
    {
        ...
        <div class="posts-wrapper list">
            <?php while ($PostData->have_posts()) : $PostData->the_post();
                if (!check_is_current_post($post_id)) :
                    $i++;
                    $maxbg = ($i == 1 && !empty($instance['is_one_maxbg'])) ? ' maxbg' : '';
                    $post_info = pm_shop_post_info();
            ?>
