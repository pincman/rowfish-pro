...
$current_chapter = !empty($_GET['chapter']) ? (int) $_GET['chapter'] : 1;
...
foreach ($video_arr as $key => $item) {
	...
	if (isset($item_exp[0]) && $item_exp[0] === '1') {
		$_free = true;
		$_vurl = (!empty($item_exp[1])) ? $item_exp[1] : '';
		$_vname = (!empty($item_exp[2])) ? $item_exp[2] : '';
	} else {
		$_free = false;
	...
	// 视频信息
	$video_data[$key] = array_merge(array('url' => '', 'name' => ''), array('url' => $_vurl, 'name' => $_vname, 'free' => $_free));
}
...
$js_video_url = '';
$current_video = $video_data[$current_chapter - 1] ?? [];
if ($IS_PAID > 0 || $current_video['free']) {
	if ($is_nologin_free) {
		...
		// echo "<div style='width: 300px;height: 300px; background-color: blue;color: #fff;margin-left: 100px;'>{$IS_PAID}</div>";
		$js_video_url = $current_video['url']; //输出视频地址
	}
} else {
	$_content .= '<div class="mb-4 text-muted">';
	if ($is_vip_post) {
		$_content .= esc_html__('本集只有订阅者才可观看', 'rizhuti-v2');
	} else {
	...
	if (!empty($shop_info['wppay_vip_auth'])) {
		$_content .= '<a href="' . get_user_page_url('vip') . '" class="btn btn-outline-success btn-rounded btn-sm mx-2 mt-lg-4"><i class="fa fa-code"></i> 马上成为本站订阅者</a>';
	}
	$_content .= '</div></div>';
}
...
if (count($video_data) > 1) {
	$_content2 =  '<p class="head-con text-muted"><i class="far fa-list-alt"></i> ' . esc_html__('教程选集', 'rizhuti-v2') . ' (' . count($video_data) . ')<b class="small text-muted">' . get_the_title() . '</b></p>';
	$_content2 .=  '';
	...
	foreach ($video_data as $key => $v) {
		...
		$_content2 .= '<li>';
		$actived = ($key == $current_chapter - 1) ? ' active' : '';
		$disabled = (!empty($js_video_url)) ? '' : 'disabled';
		$_content2 .= '<a href="' . add_query_arg("chapter", $key + 1) . '" class="switch-video' . esc_attr($actived) . '" data-index="' . ($key + 1) . '" data-url="' . $v_url . '"' . $disabled . '><span class="mr-2">P' . ($key + 1) . '</span>' . $v_name . '<i>' . $pay_note . '</i></a >';
		// $_content2 .= '<a href="' . ($key === 0 ? remove_query_arg('chapter') : add_query_arg("chapter", $key + 1)) . '" class="switch-video' . esc_attr($actived) . '" data-index="' . ($key + 1) . '" data-url="' . $v_url . '"' . $disabled . '><span class="mr-2">P' . ($key + 1) . '</span>' . $v_name . '<i>' . $pay_note . '</i></a >';
		$_content2 .= '</li>';
	}
	$_content2 .= '</ul>';
